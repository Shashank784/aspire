using System.IdentityModel.Tokens.Jwt;
using System.Security.Claims;

namespace Basket.Endpoints;

public static class BasketEndpoints
{
    public static void MapBasketEndpoints(this IEndpointRouteBuilder app)
    {
        var group = app.MapGroup("basket");

        // GET by userName
        group.MapGet("/{userName}", async (string userName, ClaimsPrincipal user, BasketService service) =>
        {
            if (!user.OwnsBasketFor(userName))
            {
                return Results.Forbid();
            }

            var shoppingCart = await service.GetBasket(userName);

            if (shoppingCart is null)
            {
                return Results.NotFound();
            }

            return Results.Ok(shoppingCart);
        })
        .WithName("GetBasket")
        .Produces<ShoppingCart>(StatusCodes.Status200OK)
        .Produces(StatusCodes.Status404NotFound)
        .RequireAuthorization("UserOnly");

        // POST (Upsert)
        group.MapPost("/", async (ShoppingCart shoppingCart, ClaimsPrincipal user, BasketService service) =>
        {
            if (!user.OwnsBasketFor(shoppingCart.UserName))
            {
                return Results.Forbid();
            }

            await service.UpdateBasket(shoppingCart);
            return Results.Created("GetBasket", shoppingCart);
        })
        .WithName("UpdateBasket")
        .Produces<ShoppingCart>(StatusCodes.Status201Created)
        .RequireAuthorization("UserOnly");

        // DELETE
        group.MapDelete("/{userName}", async (string userName, ClaimsPrincipal user, BasketService service) =>
        {
            if (!user.OwnsBasketFor(userName))
            {
                return Results.Forbid();
            }

            await service.DeleteBasket(userName);
            return Results.NoContent();
        })
        .WithName("DeleteBasket")
        .Produces(StatusCodes.Status204NoContent)
        .RequireAuthorization("UserOnly");
    }

    // A user may only touch their own basket; Admins may touch any basket.
    private static bool OwnsBasketFor(this ClaimsPrincipal user, string userName)
    {
        if (user.IsInRole("Admin"))
        {
            return true;
        }

        var owner = user.FindFirstValue(JwtRegisteredClaimNames.UniqueName) ?? user.Identity?.Name;
        return string.Equals(owner, userName, StringComparison.OrdinalIgnoreCase);
    }
}
